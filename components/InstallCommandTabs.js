...
const cliTabs = (origin) => [
  ...
  { label: "uv", code: "uv tool install goalrail" },
  { label: "pip", code: "pip install goalrail" },
  { label: "Homebrew", code: "brew install heurema/tap/goalrail" },
];

export default function InstallCommandTabs() {
  const [cliTab, setCliTab] = useState(0);
  const [origin, setOrigin] = useState("https://goalrail.dev");
  useEffect(() => {
